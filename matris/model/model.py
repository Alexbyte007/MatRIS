...
def _model_stage_profile_enabled() -> bool:
    return (
...
class MatRIS(nn.Module):
    ...
    def __init__(
        ...
        self.last_forward_profile: dict[str, float] = {}
    ...
    def forward(
        ...
        # ======== Graph processing ========
        batch_graph = _timed_model_stage(
            profile,
            "process_graphs_ms",
            (),
            lambda: process_graphs(graphs, compute_stress="s" in task),
        )
        
        # ======== Feature embedding ========
        node_feat = _timed_model_stage(
            profile,
            "atom_embedding_ms",
            (batch_graph.get("atomic_numbers"),),
            lambda: self.atom_embedding(batch_graph['atomic_numbers'] - 1),
        ) # atom type feature init (use 0 for 'H')
        edge_feat, smooth_weight = _timed_model_stage(
            profile,
            "edge_embedding_ms",
            (node_feat,),
            lambda: self.edge_embedding(graphs=batch_graph),
        ) # pairwise feature init
        threebody_feat = None 
        if len(batch_graph['line_graph_dict']['line_graph']) != 0:
            threebody_feat = _timed_model_stage(
                profile,
                "threebody_embedding_ms",
                (edge_feat,),
                lambda: self.three_body_embedding(graphs=batch_graph),
            ) # three body feature init
        elif _model_stage_profile_enabled():
            profile["threebody_embedding_ms"] = 0.0
        
        # ======== Interaction Block =======
        def run_interaction_blocks():
            local_node_feat = node_feat
            local_edge_feat = edge_feat
            local_threebody_feat = threebody_feat
            for mp_layer in self.interaction_block:
                local_node_feat, local_edge_feat, local_threebody_feat = mp_layer(
                    batch_graph=batch_graph,
                    node_feat=local_node_feat,
                    edge_feat=local_edge_feat,
                    threebody_feat=local_threebody_feat,
                    smooth_weight=smooth_weight,
                )
            return local_node_feat, local_edge_feat, local_threebody_feat

        node_feat, edge_feat, threebody_feat = _timed_model_stage(
            profile,
            "interaction_blocks_ms",
            (node_feat, edge_feat, threebody_feat),
            run_interaction_blocks,
        )
        if _model_stage_profile_enabled():
            for block in self.interaction_block:
                for key, value in getattr(block, "last_profile", {}).items():
        ...
        node_feat = _timed_model_stage(
            ...
            "readout_norm_ms",
            (node_feat,),
            lambda: self.readout_norm(node_feat),
        )
        
        total_energy = _timed_model_stage(
            profile,
            "energy_head_ms",
            (node_feat,),
            lambda: self.energy_head(batch_graph = batch_graph, node_feat = node_feat),
        )
        
        force_stress_dict = _timed_model_stage(
            profile,
            "force_stress_head_total_ms",
            (total_energy,),
            lambda: self.force_stress_head(
                batch_graph = batch_graph, 
        ...
        if _model_stage_profile_enabled():
            for key, value in getattr(self.force_stress_head, "last_profile", {}).items():
                if isinstance(value, (int, float)):
                    profile[f"force_stress.{key}"] = float(value)
        prediction.update(force_stress_dict)
        
        if "m" in task:
            magmom = _timed_model_stage(
                profile,
                "magmom_head_ms",
                (node_feat,),
                lambda: self.magmom_head(batch_graph = batch_graph, node_feat = node_feat),
            )
            prediction["m"] = magmom
        elif _model_stage_profile_enabled():
            profile["magmom_head_ms"] = 0.0
        
        def finalize_energy_prediction() -> None:
