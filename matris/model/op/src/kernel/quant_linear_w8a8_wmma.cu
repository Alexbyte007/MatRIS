...
__global__ void quant_linear_w8a8_wmma_dual_gated_tail_n128_kernel(
    ...
    float *__restrict__ core_pre_output,
    float *__restrict__ gate_pre_output,
    float *__restrict__ output,
    int64_t rows,
    int64_t core_in_features,
    int64_t gate_in_features,
    ...
    for (int idx = threadIdx.x; idx < WMMA_M * 128; idx += blockDim.x) {
        ...
        if (row < rows) {
            ...
            if (core_pre_output != nullptr) {
                core_pre_output[static_cast<int64_t>(row) * 128 + col] = core;
            }
            if (gate_pre_output != nullptr) {
                gate_pre_output[static_cast<int64_t>(row) * 128 + col] = gate;
            }
...
__global__ void quant_linear_w8a8_wmma_dual_gated_tail_n128_parallel_smooth_kernel(
    const float *__restrict__ core_input,
    const float *__restrict__ gate_input,
    const int8_t *__restrict__ core_q_weight,
    const int8_t *__restrict__ gate_q_weight,
    ...
    float *__restrict__ gate_pre_output,
    float *__restrict__ output,
    float *__restrict__ refine_node,
    int64_t rows,
    int64_t core_in_features,
    int64_t gate_in_features,
    bool core_has_bias,
    ...
    for (int idx = threadIdx.x; idx < WMMA_M * 128; idx += blockDim.x) {
        ...
        if (row < rows) {
            ...
            const float core_sigmoid = 1.0f / (1.0f + expf(-core));
            const float gate_sigmoid = 1.0f / (1.0f + expf(-gate));
            const float value = core * core_sigmoid * gate_sigmoid;
            output[static_cast<int64_t>(row) * 128 + col] = value;
            const int64_t source = source_index[row];
            const int64_t target = target_index[row];
            const float bi = base_envelope[source * 128 + col];
            const float bj = base_envelope[target * 128 + col];
            atomicAdd(&refine_node[target * 128 + col], value * bi * bj);
        }
    }
#endif
}
...
void launch_quant_linear_w8a8_wmma_dual_gated_tail_n128_parallel_smooth_kernel(
    ...
    const dim3 block(FIXED_N128_TILES * 2 * 32);
    const dim3 grid((rows + WMMA_M - 1) / WMMA_M);
    quant_linear_w8a8_wmma_dual_gated_tail_n128_parallel_smooth_kernel<<<grid, block, 0, stream>>>(
        core_input,
        gate_input,
        core_q_weight,
        gate_q_weight,
